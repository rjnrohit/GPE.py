...
if __name__ == "__main__":
    ...
    parameters = {
        ...
        'mod_g':       True, # modulate interaction
    }
    ...
    psi_f, _, _, Tstore = GPE_2d(parameters, V=V, beta=beta, psi0=psi_gs[-2], imag_t=False)
    end = time.time()
    print('runtime = %.3f' % (end-start))
    plt.figure()
    plt.imshow(Prob(psi_gs[-2]))
    plt.figure()
    #plt.plot(Prob(psi_gs[-2,128,:]))
